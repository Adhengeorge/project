from tkinter import *
from tkinter import messagebox
from PIL import Image, ImageTk
import os
import base64

def encrypt():
    message = text1.get(1.0, END)
    encode_message = message.encode("ascii")
    base64_bytes = base64.b64encode(encode_message)
    encrypt_text = base64_bytes.decode("ascii")
    
    screen1 = Toplevel(screen)
    screen1.title("encryption")
    screen1.geometry("400x200")
    screen1.configure(bg="#ed3833")
    
    Label(screen1, text="ENCRYPT", font="arial", fg="white", bg="#ed3833").place(x=10, y=0)
    text2 = Text(screen1, font="Roboto 10", bg="white", relief=GROOVE, wrap=WORD, bd=0)
    text2.place(x=10, y=40, width=380, height=150)
    
    text2.insert(END, encrypt_text)
    password = code.get()
    if password == "":
        messagebox.showerror("encryption", "Input password")
    elif password != "1234":
        messagebox.showerror("encryption", "Invalid password")

def decrypt():
    def inner_decrypt():
        password = code.get()
        if password == "1234":
            message = text1.get(1.0, END)
            decode_message = base64.b64decode(message)
            decrypt_text = decode_message.decode("ascii")
            
            screen2 = Toplevel(screen)
            screen2.title("decryption")
            screen2.geometry("400x200")
            screen2.configure(bg="#00b956")
            
            Label(screen2, text="DECRYPT", font="arial", fg="white", bg="#00b956").place(x=10, y=0)
            text3 = Text(screen2, font="Roboto 10", bg="white", relief=GROOVE, wrap=WORD, bd=0)
            text3.place(x=10, y=40, width=380, height=150)
            
            text3.insert(END, decrypt_text)
        else:
            messagebox.showerror("Error", "Incorrect password!")
    
    inner_decrypt()

def reset():
    code.set("")
    text1.delete(1.0, END)

def main_screen():
    global screen
    global code
    global text1
    
    screen = Tk()
    screen.geometry("400x400")
    
    image_path = "key.png"
    if os.path.exists(image_path):
        image = Image.open(image_path)
        image_tk = ImageTk.PhotoImage(image)
        screen.iconphoto(False, image_tk)
    else:
        messagebox.showerror("Error", f"Image file '{image_path}' not found.")
        screen.destroy()
        return
    
    screen.title("pctapp")
    
    Label(text="Enter text for encryption and decryption", fg="black", font=("Calibri", 13)).place(x=10, y=10)
    text1 = Text(font="Roboto 20", bg="white", relief=GROOVE, wrap=WORD, bd=0)
    text1.place(x=10, y=50, width=380, height=100)
    
    Label(text="Enter secret key for encryption and decryption ", fg="black", font=("calibri", 13)).place(x=10, y=170)
    code = StringVar()
    Entry(textvariable=code, width=19, bd=0, font=("arial", 25), show="*").place(x=10, y=200)
    
    Button(text="ENCRYPT", height="2", width=23, bg="#ed3833", fg="white", bd=0, command=encrypt).place(x=10, y=250)
    Button(text="DECRYPT", height="2", width=23, bg="#00b956", fg="white", bd=0, command=decrypt).place(x=200, y=250)
    Button(text="RESET", height="2", width=50, bg="#1089ff", fg="white", bd=0, command=reset).place(x=10, y=300)
    
    screen.mainloop()

main_screen()
